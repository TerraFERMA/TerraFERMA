...
\texttt{\$TF\_HOME/tutorials/poisson/simple/poisson.tfml}.  You'll be
...
example \texttt{\$TF\_HOME/tutorials/poisson/gmsh/poisson.tfml}
incorporates a mesh generated using \texttt{gmsh}.  The geometry file
for this mesh is in
\texttt{\$TF\_HOME/tutorials/poisson/gmsh/mesh/widget.geo}. To convert
for use in TF (Dolfin) use
\begin{lstlisting}[style=Bash]
$ cd $TF_HOME/tutorials/poisson/gmsh/mesh
...
\texttt{\$TF\_HOME/tutorials/poisson/mms/poisson.tfml} has two
...
also developed the \emph{TerraFERMA simulation test-harness}.  Options for a
simulation are recorded in another spud file with extension
\texttt{.shml} (simulation harness markup language), which primarily
holds pieces of python for controlling parameter sweeps and
interacting with the base \texttt{.tfml} file.  The testharness is
still undergoing active development but we have included an example in
\texttt{\$TF\_HOME/tutorials/poisson/mms/poisson.shml}.  To view this
file just use
\begin{lstlisting}[style=Bash]
 $ diamond poisson.shml
\end{lstlisting} %$
and to run it we use the utility function
...
***********  order of accuracy p= 1.98737547897
