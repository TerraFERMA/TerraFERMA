...
terms at the old and new times and permeability at the new time.
...
most peopler are not familiar with these equations}.  First,  the
...
\begin{displaymath}
  \int_{\Omega} f_{t}\Vs\cdot\grad f_{1/2}
\end{displaymath}
...
actually ambiguous as both the test function $f_{t}$ and the porosity
$f_{1/2}$ are multi-valued along the facets (thus the $*$ notation).
To fix this ambiguity, we will use an upwind scheme to select the
...
surface integrals on the external boundaries. The actual UFL for the
residual $F$ is then
\begin{lstlisting}[style=UFL]
...
# rectified normal velocity: is 0 for inflow and vn for outflow
vnabs = 0.5*(vn + abs(vn))

# body integrals for porosity
bfm = f_t*(f_i - f_n - dt*0.5*(p_i*Xi_i + p_n*Xi_n))

# body integrals from advective term
...
sfside = dt*f_t*(vnabs*f_half)  # outflows but 0 if inflow

# surface integrals over interior facets
sffacet = dt('+')*dot(vnabs('+')*f_half('+') - vnabs('-')*f_half('-'), jump(f_t))
...
The only real notation arises in the final facet integral term (which frankly I can't reconstruct and there needs to be a very clear discussion of where this term comes from plus probably a picture of a facet).  
