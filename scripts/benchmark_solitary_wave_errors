...
def geterrors(name):
    # get arguments from .bml file
    bmlfile = name+'.bml'
    libspud.load_options(bmlfile)
    
    
    # get the mesh parameters and reconstruct the mesh
    meshtype = libspud.get_option("/geometry/mesh/source[0]/name",1)
    ...
    bml_files = glob(name+'*checkpoint*.bml')
    xml_files = glob(name+'*.xml')
    dtype = [ ('time', float), ('name','S28')]
    values = []
    for i in range(len(bml_files)):
        libspud.load_options(bml_files[i])
    ...
    for i in range(len(t_file)):
        ...
        if dim == 2:
            ...
            str = "%g\t [ %g %g %g ] \t %g\t %g\t %g\t %g\t%g" % (t[-1],delta[0],delta[1],delta[2],err_min[-1],L2_f[-1],rel_error[-1],c_rel_error[-1],elapsed_time)
...
def main():
    """ Main program for calculating and generating error files
    """
    ...
    #get the output directory and chdir
    ODIR = args[0];
    if not os.path.exists(ODIR):
        print 'directory %s' % ODIR + ' doesnt exist'
        exit()
    
    os.chdir(ODIR)
    geterrors('magmawaves')
